...
func TestAccIonosCloudS3Key_ImportBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(testAccCheckIonosClouds3KeyImportConfigBasic, resourceName),
			},
			{
				ResourceName:            fmt.Sprintf("ionoscloud_s3_key.%s", resourceName),
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateIdFunc:       testAccIonosCloudS3KeyImportStateID,
...
func testAccIonosCloudS3KeyImportStateID(s *terraform.State) (string, error) {
	var importID string = ""
...
const testAccCheckIonosClouds3KeyImportConfigBasic = `
